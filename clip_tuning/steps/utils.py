...
def evaluate(qid_query_vid, similarity, scenedetect_path, gt_list, framerate):
    ...
    for (qid, query, vid), sims, gt_item in zip(qid_query_vid, similarity, gt_list):
        ...
        vid_seg_fn = os.path.join(scenedetect_path, anuj_vid + "-Scenes.csv")
        ...
        for start_time, end_time in video_timestamps:
            sim_ind_in_this_seg = list(range(int(np.floor(start_time)*framerate), min(int(np.ceil(end_time)*framerate), len(sims))))

            sim = sims[sim_ind_in_this_seg]
            if len(sim) > 0:
        ...
        assert len(pred_relevant_windows) > 0
    ...
    final_eval_metrics = eval_submission(submission, gt_list, verbose=False, match_number=True)

    return final_eval_metrics["brief"]
